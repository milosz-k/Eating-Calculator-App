        ...
        for n in range(0, len(self.current_products_list)):
            if self.remaining_proteins >= 0:
                try:
                    ...
                    self.info.append(
                        f"{self.current_products_list[n].name} has no protein, so it is impossible to fill protein by this product."
                    )
            else:
                self.info.append(
                    f"You have exceeded your proteins by {abs(self.remaining_proteins)} grams already"
                )
            ...
            if self.remaining_carbohydrates >= 0:
                try:
                    ...
                    self.info.append(
                        f"{self.current_products_list[n].name} has no carbohydrates, so it is impossible to fill carbohydrates by this product."
                    )
            else:
                self.info.append(
                    f"You have exceeded your carbohydrates by {abs(self.remaining_proteins)} grams already"
                )
            ...
            if self.remaining_fats >= 0:
                try:
                    ...
                    self.info.append(
                        f"{self.current_products_list[n].name} has no fats, so it is impossible to fill fats by this product."
                    )
            else:
                self.info.append(
                    f"You have exceeded your fats by {abs(self.remaining_proteins)} grams already"
                )
        ...
        if meal.current_day_file.my_goal.reading_complete == True:
            ...
            for product in self.current_products_list:
                objective.SetCoefficient(
                    product.grams,
                    product.proteins + product.carbohydrates + product.fats,
                )
            objective.SetMaximization()
            solver.Solve()
            for product in self.current_products_list:
                self.info.append(
                    f"{product.name}, {round(product.grams.solution_value()*100)} grams"
                )
